...
# VALIDAR ENCABEZADOS
expected_headers = [
    "Timestamp", "Nombre", "Correo", "Sabores", "Aroma", "Intensidad", "Acidez", "Cuerpo",
    "Valores", "Frecuencia", "Preparación", "Aventura", "Estilo", "Comentario", "Cafés recomendados"
]
...
if enviado:
    ...
    if any(c == "" or c == [] for c in campos_requeridos):
        ...
        cafes_previos = []
        if not historial_df.empty and "Correo" in historial_df.columns and correo in historial_df["Correo"].values:
        ...
        # Guardar recomendaciones en historial (en columna 15)
        try:
            fila = len(historial_df) + 2
            worksheet.update_cell(fila, 15, ", ".join(nombres_recomendados))
        except:
            pass
